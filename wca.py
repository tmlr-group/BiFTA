...
def main(
    ...
    # load config file
    with open(file=f"configs/{dataset_name}.yaml") as f:
    ...
    def random_crop(image: Image.Image, alpha: float) -> Image.Image:
    ...
    def custom_loader(path: str) -> torch.Tensor:
        ...
        augmented_imgs.extend(processor(random_crop(img, alpha=alpha)) for _ in range(n_samples))
